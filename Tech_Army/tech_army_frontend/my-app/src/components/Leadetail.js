...
const Dashboard = () => {
    ...
    const [itemsPerPage] = useState(6); // Number of leads per page
    ...
    useEffect(() => {
        ...
        const fetchLeads = async () => {
            try {
                ...
                // Filter users based on personstatus being "Lead"
                const filteredLeads = response.data.filter(employee => employee.personstatus === 'Lead');
    ...
    return (
        <div className="dashboard">
            ...
            <div className="employee-list">
                <h2>Team Leads</h2>
                <p>Total Leads: {leads.length}</p> {/* Display total leads */}
                <ul>
                    {currentLeads.map(lead => (
                        <li key={lead.emp_id}>
                            <Link to={`/employees/${lead.userid}`}>
